...
export function getSummaryPrompt(title: string, transcript: string, byteLimit: number) {
  ...
  return `标题: "${title.replace(/\n+/g, " ").trim()}"\n视频字幕: "${truncatedTranscript.replace(/\n+/g, " ").trim()}"\n请用中文进行视频总结:`;
}
...
export function limitTranscriptByteLength(str: string, byteLimit: number) {
  ...
  if (byteLength > byteLimit) {
    ...
    const truncatedLength = Math.floor(utf8str.length * ratio);
    const truncatedStr = utf8str.substring(0, truncatedLength);
    return decodeURIComponent(escape(truncatedStr));
  }
  return str;
}
...
export function getSmallSizeTranscripts(newTextData: SubtitleItem[], oldTextData: SubtitleItem[], byteLimit: number) {
